...
모듈화 구조의 장점을 통합한 단일 파일 버전입니다.
UI, 퀵링크, 우측 쌍둥이 버튼, 마스터 최신 다운로드 기능 및 
날짜 섞임(혼종 스냅샷) 방지 로직이 완벽하게 적용되었습니다.
"""
...
# =====================================================================
# [웹 설정 및 CSS] 페이지 세팅 및 버튼 디자인 쌍둥이 통일
# =====================================================================
st.set_page_config(page_title="EWY Quant Analytics V28", page_icon="📈", layout="wide")
...
with col_header:
    st.title("📈 EWY Quant Analytics V28")
...
def american_binomial_tree(S, K, T, r, q, sigma, opt_type, N=50):
    if T <= 0 or sigma <= 0: return max(0.0, S - K) if opt_type.startswith('C') else max(0.0, K - S)
    dt = T / N; u = np.exp(sigma * np.sqrt(dt)); d = 1 / u
    p = (np.exp((r - q) * dt) - d) / (u - d); discount = np.exp(-r * dt)
    ST = S * (d ** np.arange(N, -1, -1)) * (u ** np.arange(0, N + 1))
    V = np.maximum(0, ST - K) if opt_type.startswith('C') else np.maximum(0, K - ST)
    for i in range(N - 1, -1, -1):
        ST = ST[:-1] / d
        V = discount * (p * V[1:] + (1 - p) * V[:-1])
        V = np.maximum(V, ST - K) if opt_type.startswith('C') else np.maximum(V, K - ST)
    return V[0]
...
def calculate_bs_delta(S, K, T, r, q, sigma, opt_type):
    ...
    return np.exp(-q * T) * norm.cdf(d1) if opt_type.startswith('C') else np.exp(-q * T) * (norm.cdf(d1) - 1.0)

# =====================================================================
# [추출 엔진] Daily 옵션 데이터 수집
# =====================================================================
def extract_daily_options():
    ...
    try:
        sofr_rate = round(float(pdr.DataReader('SOFR', 'fred')['SOFR'].dropna().iloc[-1]), 2)
    except: sofr_rate = 3.65
    ...
    for exp_date in expirations:
        try:
            ...
            calls['Expiration Date'] = puts['Expiration Date'] = exp_date
    ...
    progress_bar.empty()
    df['Implied Volatility'] = [round(x, 4) if pd.notnull(x) else "" for x in iv_results]
    df['Delta'] = [round(x, 4) if pd.notnull(x) else "" for x in delta_results]
...
def apply_logic_g_down(df):
    ...
    for i, r in df.iterrows():
        ...
        if trend_up:
            ...
            elif ceiling_active and pd.notna(prev_close) and curr_close > prev_close: ceiling_active = True; phase = "⛔ 대천장 가짜 랠리 (익절 유지)"
            else:
                ceiling_active = False
    ...
    for i, r in df.iterrows():
        ...
        if trend_up:
            ...
            if is_warning and is_rollup:
                ceiling_active = True; phase = "⛔ 대천장(Ceiling) 확정 / 세력의 원월물 방어벽 롤업 완료 (전량 익절)"
            elif ceiling_active and pd.notna(prev_close) and curr_close > prev_close:
                ceiling_active = True; phase = "⛔ 대천장 유지 / 확정 이후 주가 추가 상승 (가짜 랠리 / 익절 유지)"
            else:
                ceiling_active = False
...
def apply_logic_g_up(df):
    ...
    for i, r in df.iterrows():
        ...
        if trend_up:
            ...
            elif ceiling_active and pd.notna(prev_close) and curr_close > prev_close: ceiling_active = True; phase = "⛔ 대천장 유지 / 가짜 랠리"
            else:
                ceiling_active = False
...
def load_master_data():
    ...
    if os.path.exists(master_file): return pd.read_pickle(master_file)
    return pd.DataFrame()
...
def run_quant_engine_web(version, mode, target_date=None, target_start=None, target_end=None):
    ...
    if 'recent_extracted_data' in st.session_state:
        ...
        new_df['Quote Date'] = pd.to_datetime(new_df['Quote Date']).dt.normalize()
        new_df['Expiration Date'] = pd.to_datetime(new_df['Expiration Date']).dt.normalize()
        ...
        # [V29 핵심 수정] 날짜 섞임 원천 차단 (교체 병합 로직)
        cur = master_df.copy()
        if not cur.empty:
            cur['Quote Date'] = pd.to_datetime(cur['Quote Date']).dt.normalize()
            dup = set(new_df['Quote Date']) & set(cur['Quote Date'])
            if dup:
                cur = cur[~cur['Quote Date'].isin(dup)]
        
        master_df = pd.concat([cur, new_df], ignore_index=True)
    ...
    if mode == "구간 조회" and target_start and target_end: 
        ...
        out_df = df[df['Date'] <= pd.to_datetime(target_date)].copy()
        if not out_df.empty: out_df = out_df.tail(10)
    else: 
...
# =====================================================================
# [결과 화면 1] 데이터 추출 완료 & 마스터 최신 다운로드
# =====================================================================
if "recent_extracted_data" in st.session_state:
    df_ext = st.session_state["recent_extracted_data"]
    st.success("✅ 실시간 데이터 초고속 연산 완료! (아래 버튼을 눌러 GitHub에 영구 저장하세요)")
    ...
    if not master_info.empty:
        ...
        # 마스터 데이터 가장 최근 일자 다운로드 (메인 화면 배치)
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"EWY_Master_Latest_{pd.to_datetime(latest_date).strftime('%Y%m%d')}.csv",
            mime="text/csv",
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"EWY Option 분석데이터_{quote_str}.csv", 
            mime="text/csv"
        )
    ...
    if st.button("🚀 옵션데이터 누적관리", type="primary"):
        with st.spinner("GitHub 마스터 파일에 병합 중..."):
            try:
                ...
                new_data['Quote Date'] = pd.to_datetime(new_data['Quote Date']).dt.normalize()
                new_data['Expiration Date'] = pd.to_datetime(new_data['Expiration Date']).dt.normalize()
                ...
                cur = current_master.copy()
                if not cur.empty:
                    cur['Quote Date'] = pd.to_datetime(cur['Quote Date']).dt.normalize()
                    dup = set(new_data['Quote Date']) & set(cur['Quote Date'])
                    if dup:
                        cur = cur[~cur['Quote Date'].isin(dup)]
                        
                merged_df = pd.concat([cur, new_data], ignore_index=True)
